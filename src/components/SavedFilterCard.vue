<template>
    <a-card @click="loadSavedFilter" :class="{'card-active': isSelected, 'card-link': !isSelected}">
        <a-button class="remove-button" type="link" @click="removeSavedFilter"><a-icon type="close-circle" /></a-button>
        <div class="line-item">I am looking for <br /><strong>{{ itemType }}</strong></div>
        <div class="line-item" v-if="formattedCommunities.length > 0">in <br /><strong>{{formattedCommunities.join(', ')}}</strong></div>
        <div class="line-item" v-if="formattedPrice">with a price of <br /><strong>{{formattedPrice}}</strong></div>
        <div class="line-item" v-if="item.baths > 0 || item.beds > 0 || item.home_size_range[0] !== 0 && item.home_size_range[1] !== 5000">
            with <br />
            <strong v-if="item.baths > 0">+{{item.baths}} Baths,</strong>
            <strong v-if="item.beds > 0">+{{item.beds}} Beds,</strong>
            <strong v-if="item.home_size_range[0] !== 0 && item.home_size_range[1] !== 5000">{{item.home_size_range[0]}} ~ {{item.home_size_range[1]}} Sqft,</strong>
        </div>
        <div class="line-item" v-if="formattedHomeTypes.length > 0">
            with a type of <br /><strong>{{formattedHomeTypes.join(' or ')}}</strong>
        </div>
    ...
    export default {
        ...
        computed: {
            ...
            formattedPrice() {
                ...
                return this.formatPrice(this.item.price_range[0]) + ' ~ ' + this.formatPrice(this.item.price_range[1]);
            },
